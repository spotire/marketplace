...
class SalesOrderCreditmemoSaveAfterObserver implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        $refundedShippingCharges = $creditmemo->getShippingAmount();
        ...
        foreach ($creditmemo->getAllItems() as $item) {
            ...
            if ($item->getQty()) {
                foreach ($_items as $_ordereditem) {
                    if ($item->getOrderItemId() != $_ordereditem->getItemId()) {
                        continue;
                    }
                    ...
                    if ($availableSellerFlag == 1) {
                        ...
                        if ($_ordereditem->getProductType() != 'bundle') {
                            ...
                            foreach ($bundleitems as $_bundleitem) {
                                $attributesOption = null;
                                if ($_bundleitem = \Magento\Framework\App\ObjectManager::getInstance()->get('Magento\Sales\Model\Order\Item')) {
                                    $options = $_bundleitem->getProductOptions();
                                } else {
                                    $options = $_bundleitem->getOrderItem()->getProductOptions();
                                }
                                if (isset($options['bundle_selection_attributes'])) {
                                    $attributesOption = $this->_objectManager->get(
                                ...
                                if (!$_bundleitem->getParentItem()) {
                                    ...
                                    $creditmemoItemsIds[$item->getOrderItemId()] = $item->getProductId();
                                    $creditmemoItemsQty[$item->getOrderItemId()] =
                                    intval($refundQtyArr[$_bundleitem['item_id']]);
                                    $creditmemoItemsPrice[$item->getOrderItemId()] =
                                    $creditmemoItemsPrice[$item->getOrderItemId()] +
                                    $_bundleitem->getBasePrice() * intval($refundQtyArr[$_bundleitem['item_id']]);
                                }
                            }
                        }
                    }
